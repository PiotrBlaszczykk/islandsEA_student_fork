...
- Pilot: `pilot_run/pilot_spec.json`, F1/r01 D=200, torus 12x12, 144 islands, best/plain, repeats 1-3. Full pipeline ceiling 561.5 CPUh. The production entrypoint for this one fixed matrix variant is `bash hpc_benchmarks/launch_full_torus_best_r01_3x.sh --confirm-144-and-562-cpuh`; it delegates to `pilot_run/launch_pilot.sh` so there is only one implementation. Ares rejects `sbatch` from compute-node batch jobs: the login-node launcher must pre-submit the canary, validation gate, dependency-blocked array and finalizer; the gate validates only and never submits. No CONFIRM_TORUS_200 methodology gate remains. Keep clean/pinned commit, canary verification, SCRATCH, finalizer and no-retry guards.
...
full run, but explicitly pauses the campaign after shard-position, batching
and metadata issues. That deployment status supersedes older pre-canary
summaries elsewhere in this document. Resolving ER4 alone does not resolve those separate issues.
