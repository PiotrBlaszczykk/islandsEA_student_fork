...
- Canary, gate, full array and finalizer use the absolute checkout in `ISLANDS_PROJECT_DIR`. Record the Git commit/dirty state in manifests as provenance; do not use commit equality as a cross-stage execution gate. Keep the checkout clean. Scientific validation is based on the frozen arguments, topology, resources, metrics and data integrity.
