...
This checkout targets **Athena GPU**, branch `summer_benchmarks_athena`. Its current full island runner remains CPU; the GPU backend is a separate validation/integration path.
...
- **Athena batch backend (September 14, 2026):** `benchmarks_refined/batch.py` + `batch_kernels.py` implement all 30 CEC and 10 binary functions with explicit `NumpyBatchBackend` / `CupyBatchBackend`; read `benchmarks_refined/BATCH.md` and `athena_gpu/README.md`. Canonical scalar code/data and GA adapters are unchanged. Local CPU evidence in `athena_gpu/validation_cpu.json`: 170 instances, 22016 comparisons, 1667 invalid-input checks; Python 3.12.14 / NumPy 2.3.5, **not the Athena stack**. Real CuPy tests are prepared but have NOT run for this backend. Historical F1 readiness 3167902 does not validate the new code. `athena_gpu/submit_validation.sh` prepares one 1-A100/16-CPU/15-minute validation job (maximum 0.25 GPUh), with 15 Ray CPUs + 1 driver CPU, 96/8 GiB Ray caps, pinned clean commit, scratch outputs/caches and no retries. No job was submitted by this port. Only after numerical GPU validation should another task integrate logical island shards and request batching; current IslandsEA still evaluates on CPU. Do not restore host-wide `ray stop --force` in these jobs.
