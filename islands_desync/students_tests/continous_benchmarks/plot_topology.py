...
def load_json_topology(name, island_count, random_topology_dir=None):
    source_dir = Path(
        random_topology_dir
        or os.environ.get("ISLANDS_RANDOM_TOPOLOGY_DIR")
        or RANDOM_TOPOLOGY_DIR
    )
    source_path = source_dir / f"{name}_n{island_count}.json"
...
def build_adjacency(topology, island_count, torus_width=12, random_topology_dir=None):
    ...
    if topology.startswith("rt_"):
        return load_json_topology(topology, island_count, random_topology_dir)
