...
**Kopia dla main/HPC.** Opis definicji i ich porównań historycznych pochodzi z refinementu w `help_codebase`. Wzory, dane i referencje C przeniesiono bez zmian. Wzmianki o starej fabryce `benchmark_problems.py`, rejestrach `cNN_`/`dNN_` i lokalnym Markdownzie dotyczą repozytorium pomocniczego. Main ma osobne testy integracji: 17 testów oraz dwa rzeczywiste lokalne przebiegi Ray. Szczegóły portu i walidacji: [instrukcja HPC](../../../../../hpc_benchmarks/README.md), [raport portu](../../../../../hpc_benchmarks/validation_local.json).

Pakiet `benchmarks_refined` zawiera pełne F1–F30 CEC2014 Part A oraz dziesięć dotychczasowych problemów binarnych IslandsEA. Zachowuje dobór problemów i konwencję minimalizacji. Część CEC dla D=10/30/50/100 wykorzystuje oficjalne wektory przesunięcia, macierze i permutacje, a jej wartości zostały porównane z niezależnie skompilowanym oficjalnym programem C w 1776 punktach. Wersja 1.1.0 dodaje wszystkie 30 funkcji w D=200 z osobnymi stałymi instancjami projektu; te wzory sprawdzono w kolejnych 444 punktach względem C używającego wygenerowanych danych. Część binarna zachowuje dotychczasową skalę wyników i instancję NK; testy regresji porównują ją bezpośrednio ze starszym kodem projektu.
...
| Funkcje | F1–F30 |
| Wymiary oficjalnych danych | 10, 30, 50, 100; osobne rozszerzenie projektu: 200 |
...
Źródłem tych parametrów są sekcje hybryd w raporcie i funkcje `hf01`–`hf06` w [oficjalnej dystrybucji](https://raw.githubusercontent.com/P-N-Suganthan/CEC2014/98488087d590c29aaded9978ccfe2a356d10dd63/cec14-c-code.zip). Żaden ze wspieranych wymiarów 10/30/50/100/200 nie tworzy pustych grup.
...
| Wymiary | Dowolny wymiar wymagałby nowych danych instancji | Oficjalne 10,30,50,100 oraz jawnie oznaczone wygenerowane instancje 200D |
...
## Rozszerzenie do 200 wymiarów

Wszystkie `r01_...`–`r30_...` akceptują D=200. Wybór następuje tak samo jak dla innych wymiarów: przez API, pole `variables` w konfiguracji batcha albo `ISLANDS_NUMBER_OF_VARIABLES=200`. Nazwy funkcji i domyślne D=30 pozostają takie same. Wymiar jest częścią ścieżki wyniku oraz manifestu, więc 100D i 200D nie dzielą tego samego katalogu problemu.

Instancje 200D noszą identyfikator `islandsea-cec2014-d200-v1` i mają `official_cec2014_instance=false`. Nie występują w oficjalnym archiwum CEC2014. Zachowują wzory, skale, biasy, granice, proporcje hybryd i składniki kompozycji; nowe są wektory i macierze definiujące instancje. Oficjalne pliki danych D=10/30/50/100 pozostają bez zmian.

Generator `tools/prepare_cec200_data.py` ma następującą jawną specyfikację:

| Element | Definicja rozszerzenia 200D |
|---|---|
| Seed główny | `20260911` |
| Generator | NumPy PCG64, `SeedSequence([seed,200,fid,component,stream])` |
| Przesunięcia | Niezależne współrzędne o rozkładzie jednostajnym w `[-80,80)` |
| Powiązania współrzędnych | Dziesięć bloków po 20 współrzędnych; losowa wspólna permutacja wierszy i kolumn macierzy |
| Macierz bloku | `U @ diag(geomspace(1,2,20)) @ V.T`; U,V z QR niezależnych macierzy standardowo normalnych, z ustaloną konwencją znaków |
| Liczba uwarunkowania | 2 dla każdego bloku i całej macierzy; nie jest to sama macierz ortogonalna |
| Permutacja hybrydy | Osobny strumień losujący permutację 1,…,200 dla danego składnika |
| Rozdzielenie strumieni | Osobne dla przesunięcia, czynników macierzy, kolejności zmiennych i permutacji hybrydy |
| Liczba zestawów składników | 1 dla F1–F22, 10 dla F23–F30; kompozycje wykorzystują pierwsze 3 lub 5 zgodnie z ich wzorem |

Blokowe powiązania nawiązują do opisu częściowo powiązanych zmiennych w raporcie CEC. Konkretny podział 10×20 i powyższy generator są decyzją projektu, a nie odzyskanym generatorem autorów CEC. Nie jest to także powielenie wektora 100D ani osadzenie tego samego problemu w dodatkowych zerowych współrzędnych. W F8/F10 oraz składnikach, w których C wyłącza rotację, macierz nadal jest pomijana zgodnie z definicją funkcji.

Gotowa paczka `data/d200/cec2014.npz` zajmuje około 4,1 MB. Worker wczytuje tylko potrzebne zestawy i nie generuje danych podczas eksperymentu. Manifest zapisuje hash paczki, parametry generatora, hash jego kodu i środowisko generowania. Wspólny NPZ zapewnia te same wartości wejściowe na wszystkich wyspach. Sam seed nie gwarantuje identyczności ostatnich bitów po ponownym QR w innej wersji LAPACK; dlatego w badaniach należy zachować paczkę danych i jej hash.

Referencję 200D wygenerowano oddzielnie: program C otrzymał te dane w plikach tekstowych z 17 cyframi znaczącymi. Poza dotychczasowymi poprawkami przenośności dodano tylko `nx==200` do warunku dopuszczającego wymiar. Wzory C pozostały bez zmian. Test w 444 punktach sprawdza zgodność wykonania wzorów dla nowej instancji; nie uprawnia do nazywania tych danych oficjalnym CEC2014 200D. Odpowiedni hash instancji jest powiązany z wynikami referencyjnymi.

Dodanie 200D umożliwia dobór trudności do projektowego `zakres_badan.pdf`. Nie przesądza, że każda funkcja powinna być badana w tym wymiarze lub że przy populacji 16 będzie wykazywała użyteczny postęp. Wymiar pozostaje parametrem dobieranym przed porównywaniem topologii i strategii.
...
| Rozszerzenie 200D, wzory C z wygenerowanymi danymi | 444 punkty wszystkich 30 funkcji oraz ich 30 optimów | Te same tolerancje co dla oficjalnych wymiarów |
| Dane 200D i metadane | Granice przesunięć, liczby uwarunkowania, rozmiary bloków, permutacje, niezmienność danych, odrzucenie uszkodzonego NPZ | Poprawne; manifest odróżnia rozszerzenie od danych oficjalnych |
...
| Regresja wobec starej binarnej fabryki, wykonana w help | 20 losowych ciągów × 10 funkcji, n=60 | Identyczne wartości float; plik funkcji binarnych przeniesiono bez zmian |
| Adaptery i operatory jMetalPy | Wszystkie 40 funkcji domyślnych oraz 30 funkcji 200D; 8 osobników, 4 potomków, 24 ewaluacje | Pełne krótkie przebiegi zakończone |
| Serializacja | `pickle` każdego z powyższych 70 adapterów i ponowna ewaluacja | Zachowana wartość |
| Aktywna fabryka i algorytm wyspowy | F29 30D, F30 200D i NK; prawdziwy pierwszy krok GA; lokalne zastępstwo komunikacji jednej wyspy | Poprawny krok i manifest zgodny z parametrami |
...
Oficjalny C został skompilowany z MSVC x64 z `/O2 /fp:precise`. Dla oficjalnych wymiarów jedynymi poprawkami źródła są usunięcie nieużywanego `WINDOWS.H` i zamiana formatu wejściowego `%Lf` na `%lf` przy odczycie do `double*`. Rozszerzenie 200D dodatkowo dopuszcza ten wymiar i używa nowych danych, jak opisano wyżej. Generator zapisuje komendę kompilacji, hash oryginalnego C i pochodzenie archiwum wraz z wektorami. Obsługuje także g++ z `-O2 -fno-fast-math`, ale dołączone wyniki odniesienia pochodzą z wykonanej kompilacji MSVC.

Pełny lokalny wynik w main to **17 testów, 0 błędów, 0 niepowodzeń i 0 pominięć**, w środowisku Windows 11, Python 3.12.14, NumPy 1.26.4, jMetalPy 1.5.5, Ray 2.31.0, SciPy 1.14.1 i scikit-learn 1.5.2. Testy integracji dostosowano do fabryki main, dodając kontrolę odległości binarnej i preflightu HPC; historyczna regresja wobec starej fabryki pozostaje w help. Testy są krótkie i można je powtórzyć przez `python -m islands_desync.geneticAlgorithm.utils.benchmarks_refined --validate`. Wariant `--core-only` ma 12 testów i nie obejmuje integracji z GA.
