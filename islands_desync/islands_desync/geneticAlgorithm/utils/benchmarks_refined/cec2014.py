"""CEC 2014 Part A functions and a fixed IslandsEA 200D extension.

The public suite contains F1--F30 with official data for D=10,30,50,100.
D=200 uses separately identified, bundled generated data, not official CEC
instances. The formulas are the same at all supported dimensions.
...
OFFICIAL_DIMENSIONS = (10, 30, 50, 100)
SUPPORTED_DIMENSIONS = OFFICIAL_DIMENSIONS + (200,)
_DATA_FILE = Path(__file__).with_name("data") / "cec2014.npz"


def _data_file(extended=False):
    return _DATA_FILE.parent / "d200" / _DATA_FILE.name if extended else _DATA_FILE
...
@lru_cache(maxsize=2)
def _verified_provenance(extended=False):
    path = _data_file(extended)
    ...
    if hashlib.sha256(path.read_bytes()).hexdigest() != manifest["data_sha256"]:
        raise RuntimeError(f"CEC2014 data checksum mismatch; restore the bundled dataset: {path}")
    return manifest
...
def _load_instance(fid, dimension):
    ...
    _verified_provenance(dimension == 200)
    path = _data_file(dimension == 200)
    ...
    try:
        ...
        raise RuntimeError(f"Cannot load CEC2014 F{fid}, D={dimension} from {path}") from error
...
class CEC2014:
    ...
    def optimum_position(self):
        """A copy of the first instance shift vector (the global optimum)."""
        return self._shifts[0].copy()

    def metadata(self):
        extended = self.dimension == 200
        source = _verified_provenance(extended)
        result = {"cec_function_id": self.fid, "dimension": self.dimension,
                ...
                "official_cec2014_instance": not extended,
                "instance": source["instance_id"] if extended else "official fixed CEC2014 Part A data"}
        if extended:
            result.update({"generation": source["generation"],
                           "generator_sha256": source["generator_sha256"],
                           "source_relationship": source["source_relationship"]})
        # Keep mutable manifest dictionaries private to the cached loader.
