...
def _create_problem(problem_name: str, number_of_variables: int):
    ...
    if normalized in ("trap5", "deceptive_trap", "trp5"):
        return myDefProblems.DeceptiveTrap5(number_of_variables)
    if normalized in ("nk_k4", "nk4", "nk4b"):
        return myDefProblems.AdjacentNKLandscape(number_of_variables, k=4)
...
def create_algorithm_hpc(
    ...
    is_binary_problem = isinstance(problem, BinaryProblem)
    ...
    genetic_island_algorithm = GeneticIslandAlgorithm(
        ...
        # crossover=SPXCrossover(1.0),
        mutation=mutation, #number_of_variables()
        # 0.5, 9.0),
