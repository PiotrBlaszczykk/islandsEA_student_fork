...
def main():
    parser = argparse.ArgumentParser(
        description="Run a selected range of local_matrix_computation batch JSON files sequentially."
    )
    parser.add_argument(
        "batch_dir",
        help=(
    ...
    parser.add_argument(
        "--limit-jobs",
        type=int,
        help="Pass --limit N to each batch, useful for probing many batch files.",
    )
    parser.add_argument(
        "--only",
        help="Pass --only TEXT to each batch.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
    ...
    parser.add_argument(
        "--log-dir",
        help="Directory for per-batch logs. Default: local_matrix_computation/batch_logs/<group>.",
    )
