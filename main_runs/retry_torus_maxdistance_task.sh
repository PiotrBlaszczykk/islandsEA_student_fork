...
# Explicitly retry one failed element of an existing torus_maxdistance campaign and
...
[[ ! -e "$CAMPAIGN_DIR/torus_maxdistance.tar.gz" ]] || {
...
RETRY_SUBMISSION=$(sbatch --parsable \
    ...
    --array="$TASK_ID" \
    --output="$CAMPAIGN_DIR/logs/torus-maxdistance-retry-%A_%a.out" \
    --error="$CAMPAIGN_DIR/logs/torus-maxdistance-retry-%A_%a.err" \
    --export="ALL,ISLANDS_PROJECT_DIR=${PROJECT_DIR},ISLANDS_CAMPAIGN_DIR=${CAMPAIGN_DIR},ISLANDS_VENV_DIR=${VENV_DIR},ISLANDS_CAMPAIGN_STRATEGY=maxDistance,ISLANDS_CAMPAIGN_ARRAY_JOB_ID=${CAMPAIGN_ARRAY_JOB_ID}" \
...
cat <<EOF
TORUS_MAXDISTANCE_RETRY_TASK=$TASK_ID
TORUS_MAXDISTANCE_RETRY_ARRAY_JOB_ID=$RETRY_ARRAY_JOB_ID
TORUS_MAXDISTANCE_RETRY_FINALIZER_JOB_ID=$FINALIZER_JOB_ID
