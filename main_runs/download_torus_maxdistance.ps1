param(
    ...
    [string]$RemoteCampaignDir = '/net/afscra/people/plgblaszczykk/torus_maxdistance',
    [string]$Destination = '',
    [switch]$Extract
)
...
if (-not $Destination) {
    ...
    $Destination = Join-Path $workspace 'artifacts/torus_maxdistance'
}

$archive = Join-Path $Destination 'torus_maxdistance.tar.gz'
$checksum = "$archive.sha256"
$directory = Join-Path $Destination 'torus_maxdistance'
...
& scp "${Remote}:$RemoteCampaignDir/torus_maxdistance.tar.gz*" $Destination
...
if ($fields.Count -ne 2 -or $fields[1] -ne 'torus_maxdistance.tar.gz' -or
...
Write-Host "TORUS_MAXDISTANCE_SHA256_OK=$archive"
...
if ($Extract) {
    ...
    Write-Host 'TORUS_MAXDISTANCE_NESTED_SHA256_OK=120'
    Write-Host "TORUS_MAXDISTANCE_DIRECTORY=$directory"
}
Write-Host "TORUS_MAXDISTANCE_DOWNLOAD_OK=$archive"
