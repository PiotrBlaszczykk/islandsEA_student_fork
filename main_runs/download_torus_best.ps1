param(
    ...
    [string]$RemoteCampaignDir = '/net/afscra/people/plgblaszczykk/torus_best',
    [string]$Destination = '',
    [switch]$Extract
)
...
if (-not $Destination) {
    ...
    $Destination = Join-Path $workspace 'artifacts/torus_best'
}

$archive = Join-Path $Destination 'torus_best.tar.gz'
$checksum = "$archive.sha256"
$directory = Join-Path $Destination 'torus_best'
...
& scp "${Remote}:$RemoteCampaignDir/torus_best.tar.gz*" $Destination
...
if ($fields.Count -ne 2 -or $fields[1] -ne 'torus_best.tar.gz' -or
...
Write-Host "TORUS_BEST_SHA256_OK=$archive"
...
if ($Extract) {
    ...
    Write-Host 'TORUS_BEST_NESTED_SHA256_OK=120'
    Write-Host "TORUS_BEST_DIRECTORY=$directory"
}
Write-Host "TORUS_BEST_DOWNLOAD_OK=$archive"
