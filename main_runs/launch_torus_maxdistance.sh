...
# Submit the fixed torus/maxDistance campaign: 40 benchmarks x 3 independent repeats.
# This file runs on the login node. All sbatch calls deliberately happen here.
set -euo pipefail
export ISLANDS_CAMPAIGN_STRATEGY=maxDistance

SCRIPT_DIR=$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)
PROJECT_DIR=$(cd -- "$SCRIPT_DIR/.." && pwd -P)
VENV_DIR="${ISLANDS_VENV_DIR:-${HOME}/venvs/islands-ray}"
MAX_PARALLEL="${TORUS_MAXDISTANCE_MAX_PARALLEL:-3}"
: "${SCRATCH:?SCRATCH must be available on Ares}"
CAMPAIGN_DIR="${TORUS_MAXDISTANCE_CAMPAIGN_DIR:-$SCRATCH/torus_maxdistance}"
...
[[ "$MAX_PARALLEL" =~ ^[1-3]$ ]] || {
    echo "TORUS_MAXDISTANCE_MAX_PARALLEL must be 1, 2 or 3" >&2
    exit 2
}
...
for benchmark in "${BENCHMARKS[@]}"; do
    "$VENV_DIR/bin/python" "$PROJECT_DIR/hpc_benchmarks/run_benchmark.py" \
        ...
        --strategy maxDistance --acceptance plain --repeat 1 --seed 20260912 \
        --startup-timeout 300 --actor-startup-timeout 300 --dry-run >/dev/null
done
echo "TORUS_MAXDISTANCE_PREFLIGHT_OK benchmarks=40 tasks=120"
...
ARRAY_SUBMISSION=$(sbatch --parsable \
    --job-name=torus-maxdistance \
    --array="1-120%${MAX_PARALLEL}" \
    --output="$CAMPAIGN_DIR/logs/torus-maxdistance-%A_%a.out" \
    --error="$CAMPAIGN_DIR/logs/torus-maxdistance-%A_%a.err" \
    --export="ALL,ISLANDS_PROJECT_DIR=${PROJECT_DIR},ISLANDS_CAMPAIGN_DIR=${CAMPAIGN_DIR},ISLANDS_VENV_DIR=${VENV_DIR},ISLANDS_CAMPAIGN_STRATEGY=maxDistance" \
    "$SCRIPT_DIR/run_torus_best_array.sh")
ARRAY_JOB_ID="${ARRAY_SUBMISSION%%;*}"

FINALIZER_SUBMISSION=$(sbatch --parsable \
    --job-name=torus-maxdistance-finalize \
    --dependency="afterany:${ARRAY_JOB_ID}" \
    --output="$CAMPAIGN_DIR/logs/torus-maxdistance-finalize-%j.out" \
    --error="$CAMPAIGN_DIR/logs/torus-maxdistance-finalize-%j.err" \
    --export="ALL,ISLANDS_PROJECT_DIR=${PROJECT_DIR},ISLANDS_CAMPAIGN_DIR=${CAMPAIGN_DIR},ISLANDS_VENV_DIR=${VENV_DIR},ISLANDS_CAMPAIGN_STRATEGY=maxDistance" \
...
cat <<EOF
TORUS_MAXDISTANCE_ARRAY_JOB_ID=$ARRAY_JOB_ID
TORUS_MAXDISTANCE_FINALIZER_JOB_ID=$FINALIZER_JOB_ID
TORUS_MAXDISTANCE_CAMPAIGN_DIR=$CAMPAIGN_DIR
TORUS_MAXDISTANCE_EXPECTED_RUNS=120
TORUS_MAXDISTANCE_MAX_PARALLEL=$MAX_PARALLEL
TORUS_MAXDISTANCE_ALLOCATION_CEILING_CPUH=40320
...
Final archive: $CAMPAIGN_DIR/torus_maxdistance.tar.gz
EOF
